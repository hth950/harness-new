...
import { codexCostUsd } from './pricing.mjs';
import { writeReview, VERDICTS } from './cross-review.mjs';

// Absolute path to the Homebrew-installed codex binary; falls back to 'codex' on
// PATH if the pinned path is not present (portable across machines).
const CODEX_BIN = '/opt/homebrew/bin/codex';

// DEFAULT LIVE codexRunner — shells the REAL codex CLI to EDIT the worktree.
//
// This is the production runner the /harness skill (or a live smoke test) passes
// as opts.codexRunner. It is OPT-IN: runCodexWorker does NOT default to it, so unit
// tests never invoke the real CLI (they inject their own mock runner).
//
// Contract (matches runCodexWorker's codexRunner): edits files in `worktree` as a
// SIDE EFFECT (the orchestrator owns the diff afterward); returns { tokens, stdout }
// where tokens is parsed from the CLI's trailing "tokens used N" line.
//
//   codexRunner({ prompt, promptFile, worktree, model, round }) -> { tokens, stdout }
//
// The model is PINNED (defaults to DEFAULT_CODEX_MODEL = gpt-5.5) so the MCP
// fallback chain can't downgrade it to a rejected model. --full-auto lets codex
// edit the worktree without per-action approval; -C <worktree> + cwd:worktree scope
// it to the isolated tree. A 1-hour timeout matches the codex one-shot ceiling.
export function defaultLiveCodexRunner({ prompt, promptFile, worktree, model, round } = {}) {
  const resolvedPrompt = resolveLivePrompt({ prompt, promptFile });
  const m = model || DEFAULT_CODEX_MODEL;
  const args = ['exec', '--full-auto', '-C', worktree, '-m', m, resolvedPrompt];
  const bin = existsSync(CODEX_BIN) ? CODEX_BIN : 'codex';
  const stdout = execFileSync(bin, args, {
    cwd: worktree,
    encoding: 'utf8',
    timeout: 3_600_000, // <= 1 hour (codex one-shot ceiling)
    maxBuffer: 64 * 1024 * 1024,
    stdio: ['ignore', 'pipe', 'pipe'],
  });
  const tokens = parseCodexTokens(stdout);
  return { tokens, stdout, round };
}

// Resolve the live runner's prompt from an inline `prompt` or a `promptFile` path.
function resolveLivePrompt({ prompt, promptFile }) {
  if (typeof prompt === 'string' && prompt.length > 0) return prompt;
  if (typeof promptFile === 'string' && promptFile.length > 0 && existsSync(promptFile)) {
    return readFileSync(promptFile, 'utf8');
  }
  throw new Error('defaultLiveCodexRunner requires a non-empty `prompt` or an existing `promptFile`');
}
...
export async function runCodexWorker(runDir, agentId, opts = {}) {
  const {
    ...
    // Codex billing mode for cost attribution. 'subscription' (DEFAULT — the user's
    // ChatGPT-account Codex) => codex dollar cost is 0 (flat billing). 'api' (OpenAI
    // API key) => metered via pricing.codexCostUsd's blended approximation. Wired
    // from harness-config (getCodexBillingMode) in a live run.
  ...
  for (let round = 1; round <= maxRounds; round++) {
    ...
    // --- record codex cost from tokens (billing-mode aware) -----------------
    // codexCostUsd returns 0 in 'subscription' mode (flat ChatGPT-account billing)
    // and the blended-rate approximation in 'api' mode. We still record a 0-dollar
    // spend in subscription mode so the ledger reflects the round occurred (and a
    // spawns delta could ride along), but it never inflates the dollar ceiling.
    if (tokens && tokens > 0) {
      const usd = codexCostUsd(model, tokens, codexBillingMode);
      if (usd > 0) {
