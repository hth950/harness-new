...
function _isAllowed(path, allow) {
  for (const rule of allow) {
    if (pathMatchesRule(path, rule)) return true;
  }
  return false;
}

// THE single allowlist matching predicate (plan §5.5). Does a concrete `path`
// match an allowlist `rule`? This is the ONE place the prefix/glob semantics
// live so the round-merge gate (validateTouched/_isAllowed) and the ownership
// PARTITION gate (rulesCanOverlap in ownership.mjs) can NEVER drift apart.
//
// Rule forms (matching the original _isAllowed semantics exactly):
//   - exact:        rule === path
//   - "dir/**":     path startsWith "dir/"      (recursive directory glob)
//   - "prefix*":    path startsWith "prefix"    (bare-star prefix)
//   - "dir/":       path startsWith "dir/"      (directory prefix)
export function pathMatchesRule(path, rule) {
  if (typeof rule !== 'string' || typeof path !== 'string') return false;
...
// Do two allowlist rules OVERLAP — i.e. is there ANY concrete path both could
// match? This is the predicate the ownership PARTITION gate needs (two tasks
// overlap when a rule from each can both match a common path). It MUST be derived
// from the SAME pathMatchesRule semantics the merge gate uses, so the partition
// guarantee matches the real allowlist authorization (HIGH-PN: a directory-prefix
// entry like 'src/' and a nested file 'src/a.js' overlap because 'src/' authorizes
// 'src/a.js' under pathMatchesRule, even though they are not string-equal).
//
// Cases (each rule reduced to {exact-path | prefix-that-authorizes-a-subtree}):
//   - exact vs exact:    overlap iff equal.
//   - exact vs prefix:   overlap iff the prefix authorizes the exact path
//                        (pathMatchesRule(exactPath, prefixRule)).
//   - prefix vs prefix:  two subtree prefixes overlap iff one is a prefix of the
//                        other (the shorter subtree contains the longer's root),
//                        e.g. 'src/' vs 'src/sub/' both cover 'src/sub/x'.
export function rulesCanOverlap(a, b) {
  if (typeof a !== 'string' || typeof b !== 'string') return false;
  const pa = _rulePrefix(a);
  const pb = _rulePrefix(b);
  // Both are exact paths: overlap only on equality.
  if (pa == null && pb == null) return a === b;
  // One exact, one prefix: the prefix must authorize the exact path.
  if (pa == null) return pathMatchesRule(a, b); // a exact, b prefix
  if (pb == null) return pathMatchesRule(b, a); // b exact, a prefix
  // Both prefixes (subtree roots): overlap iff one subtree contains the other.
  return pa.startsWith(pb) || pb.startsWith(pa);
}

// Reduce a rule to the directory/string prefix that defines the subtree it
// authorizes, or null when the rule is an EXACT path (no glob/prefix). Mirrors
// pathMatchesRule's branch order so the two stay consistent.
function _rulePrefix(rule) {
  if (rule.endsWith('/**')) return rule.slice(0, -2); // keep trailing "/"
  if (rule.endsWith('*')) return rule.slice(0, -1);
  if (rule.endsWith('/')) return rule;
  return null; // exact path
}
