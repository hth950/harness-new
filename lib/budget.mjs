...
// max_spawns null = no spawn-count cap (unbounded) unless set (LOW-BUD).
...
export function loadBudget(runDirPath) {
  ...
  return {
    ...
    max_spawns: base.max_spawns ?? null,
...
// Is the run over (or exactly at) its spawn-count cap (LOW-BUD)? No cap => never
// over. This is a DETERMINISTIC count ceiling that complements the cost ceiling:
// once `spawns` (the summed ledger count) reaches max_spawns, further spawns are
// denied even if cost is still under the dollar ceiling.
export function isOverSpawnCap(budget) {
  if (budget.max_spawns == null) return false;
  return (budget.spawns ?? 0) >= budget.max_spawns;
}

// Can we spawn a new agent / start a new round? Returns false (denied) when EITHER
// the cost ceiling is already met/exceeded OR the spawn-count cap (max_spawns) is
// met/exceeded, emitting a budget_alert in either case. Returns true otherwise.
export function canSpawn(runDirPath, { agentId = ORCHESTRATOR_AGENT_ID } = {}) {
  const budget = loadBudget(runDirPath);
  const overCeiling = isOverCeiling(budget);
  const overSpawnCap = isOverSpawnCap(budget);
  if (overCeiling || overSpawnCap) {
    const msg = overCeiling
      ? `spawn denied: spend ${totalSpend(budget).toFixed(4)} >= ceiling ${budget.ceiling_usd}`
      : `spawn denied: spawns ${budget.spawns} >= max_spawns ${budget.max_spawns}`;
