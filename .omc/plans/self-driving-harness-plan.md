...
### 실사용화 (Productionization) — ✅ 완료 (2026-06-10) → **🎉 실제 라이브 동작 증명**
**증거**: ROOT **78/78**, DASHBOARD **20/20** (98). + **라이브 스모크 성공**: 실제 `codex exec --full-auto`(gpt-5.5, ChatGPT 계정)가 임시 git 레포에 `add.mjs` 생성 → 오케스트레이터가 `git diff`로 patch 소유 → 리뷰 승인 → integration 머지(~24s, finalState=merged, codex_cost=0 구독).

**산출/변경**:
- `lib/pricing.mjs` — **실단가**(2026-06): Claude Opus4.8 $5/$25·Sonnet4.6 $3/$15·Haiku4.5 $1/$5, OpenAI gpt-5.5 $5/$30·gpt-5 $0.625/$5 (per 1M in/out). `priceFor/costUsd/costUsdFromTotal/codexCostUsd`. **codex `subscription`=0 / `api`=blended**.
- `lib/harness-config.mjs` — 중앙 설정(`harness.config.json` + env): `budget.ceiling_usd`(20)/`max_spawns`(30), `maxParallel`(5), `claudeModel`(opus-4-8), `codexModel`(gpt-5.5), `codexBillingMode`(subscription), `priceOverrides`.
- `lib/codex-round-runner.mjs` — `defaultLiveCodexRunner`(실 codex CLI로 worktree 편집, opt-in; 테스트는 주입 mock) + billing-aware 비용.
- `lib/codex-cost.mjs` — placeholder 제거, pricing.mjs 위임. `orchestrator.mjs` — billingMode 스레딩.
- `README.md` — 전체 문서(아키텍처·파이프라인·안전모델·설정·비용·검증된 제약·실행법). `skills/harness/SKILL.md` 갱신.

**중요(실사용)**: 사용자의 Codex는 ChatGPT 계정 구독제 → 예산의 실질 비용은 Claude 측. 그래서 `codexBillingMode` 기본 `subscription`(codex_cost 0), API 키 사용 시 `api`로 표 적용.

**현재 상태**: Phase 0/1/1.5/2 + 실사용화 완료, 모두 github.com/hth950/harness-new main에 머지. 라이브 동작 검증됨. 남은 것: Phase 3(선택).
